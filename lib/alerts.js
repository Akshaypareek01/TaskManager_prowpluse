...
/** Roster id used for team-wide broadcast alerts (announcements). */
export const BROADCAST_MEMBER_ID = "__broadcast__";
...
/**
 * Insert a team-wide announcement alert visible to everyone.
 * @param {import("drizzle-orm/node-postgres").NodePgDatabase} db
 * @param {{ title: string, description: string, authorName: string }} params
 */
export async function insertAnnouncement(db, { title, description, authorName }) {
  const id = newAlertId("ann");
  await insertAlert(db, {
    id,
    type: "announcement",
    memberId: BROADCAST_MEMBER_ID,
    taskId: null,
    title,
    announcedByName: authorName,
    message: description,
    read: false,
