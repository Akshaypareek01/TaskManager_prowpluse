...
import { BACKLOG_OVERDUE_DAYS } from "./analytics.js";
 ...
 * @param {string} today
 * @param {Date} now
 * @returns {{ overdueCount: number, backlogCount: number, badgeCount: number, pendingToday: number, completedToday: number }}
 */
export function memberTaskCounts(allTasks, memberId, today, now) {
  ...
  return { overdueCount, backlogCount, badgeCount, pendingToday, completedToday };
}
