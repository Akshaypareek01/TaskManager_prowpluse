...
import { ensureWeeklyReportIfNeeded } from "./reportStore.js";
...
function mapAlertToApi(a, taskTitle = null, memberLookup = {}) {
  ...
  const isBroadcast = a.memberId === BROADCAST_MEMBER_ID;
  return {
    id: a.id,
    type: a.type,
    member: m
      ...
      : isBroadcast
        ? {
            id: BROADCAST_MEMBER_ID,
            name: "All team",
            color: "#f59e0b",
            initials: "ALL",
          }
        : { id: a.memberId, name: a.memberId, initials: initials(a.memberId) },
    title: a.title || null,
    announcedBy: a.announcedByName || null,
    message: a.message,
    taskId: a.taskId,
    taskTitle,
...
export async function getAlerts({
  ...
  if (memberId !== "all") {
    conditions.push(
      or(eq(alerts.memberId, memberId), eq(alerts.memberId, BROADCAST_MEMBER_ID))
    );
  }
...
export async function getState({ forceAlerts = false } = {}) {
  ...
  await ensureWeeklyReportIfNeeded(now);
  ...
  const members = registered.map((m) => {
    ...
    const broadcastUnread = alertRows.filter(
      (a) => a.memberId === BROADCAST_MEMBER_ID && !a.read
    ).length;
    const unreadAlertCount =
      alertRows.filter((a) => a.memberId === m.id && !a.read).length +
      broadcastUnread;
    return {
      ...toMemberView(m),
      ...counts,
...
const MAX_ANNOUNCEMENT_BODY = 600;

/**
 * Post a team-wide announcement alert.
 * @param {{ title: string, description: string, authorName: string }} params
 * @returns {Promise<object>}
 */
export async function createAnnouncement({ title, description, authorName }) {
  ...
  if (trimmedTitle.length > MAX_ANNOUNCEMENT_TITLE) {
    throw new Error(`Title must be under ${MAX_ANNOUNCEMENT_TITLE} characters.`);
  }
  if (trimmedDesc.length > MAX_ANNOUNCEMENT_BODY) {
    throw new Error(`Description must be under ${MAX_ANNOUNCEMENT_BODY} characters.`);
  }

  const db = getDb();
  await insertAnnouncement(db, {
    title: trimmedTitle,
    description: trimmedDesc,
    authorName: trimmedAuthor,
  });

  return getState({ forceAlerts: true });
}
