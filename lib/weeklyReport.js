...
import {
  appDayStr,
  appLocalDateTimeFromDayStr,
  appLocalDateTimeToDate,
  getAppTimezoneParts,
} from "./appTimezone.js";
...
Include every team member listed in the stats (exclude anyone not in the input).`;
...
/**
 * Monday–Sunday bounds for the week containing a calendar day (app timezone).
...
export function getWeekBounds(date) {
  const dayStr =
    typeof date === "string" ? date : appDayStr(date instanceof Date ? date : new Date(date));
  const d = parseLocalDay(dayStr);
  d.setHours(12, 0, 0, 0);
  const weekStart = addDays(dayStr, -daysFromMonday(d));
...
export function aggregateWeekStats(tasks, members, weekStart, weekEnd) {
  ...
  const reportMembers = members.filter((m) => !isBossMember(m));

  const memberStats = reportMembers.map((m) => {
...
/**
 * Next Monday 9:00 AM in app timezone (when the next report generates).
 * @param {Date} [now]
 * @returns {Date}
 */
export function getNextReportDate(now = new Date()) {
  const today = appDayStr(now);
  const parts = getAppTimezoneParts(now);
  const dow = parseLocalDay(today).getDay();

  if (dow === 1 && parts.hour < REPORT_GENERATION_HOUR) {
    const [year, month, day] = today.split("-").map(Number);
    return appLocalDateTimeToDate(year, month, day, REPORT_GENERATION_HOUR, 0);
  }

  let targetDay = today;
  if (dow === 1) {
    targetDay = addDays(today, 7);
  } else {
    const daysUntilMonday = dow === 0 ? 1 : 8 - dow;
    targetDay = addDays(today, daysUntilMonday);
  }
...
/**
 * Whole calendar days until the next scheduled report (app timezone).
 * @param {Date} [now]
 * @returns {number}
 */
 ...
 * Due from Monday 9 AM IST after the report week ends through the following days (catch-up).
 * @param {Date} [now]
 * @returns {boolean}
 */
export function shouldGeneratePreviousWeekReport(now = new Date()) {
  const { weekEnd } = getPreviousWeekBounds(now);
  const generationDay = addDays(weekEnd, 1);
  const generationTime = appLocalDateTimeFromDayStr(
    generationDay,
    REPORT_GENERATION_HOUR,
    0
  );
  return now.getTime() >= generationTime.getTime();
}
