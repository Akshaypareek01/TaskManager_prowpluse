...
export function reportToApi(row) {
  ...
  const members = (content.members || []).filter((m) => !isBossMember({ name: m.name }));
  return {
    id: row.id,
...
/**
 * Auto-generate the previous week's report when due and missing.
 * Safe to call on every app poll — idempotent and cheap when report exists.
 * @param {Date} [now]
 * @returns {Promise<{ report: object, created: boolean }|null>}
 */
export async function ensureWeeklyReportIfNeeded(now = new Date()) {
  if (!shouldGeneratePreviousWeekReport(now)) return null;

  const { weekStart, weekEnd } = getPreviousWeekBounds(now);
  const existing = await getReportByWeekStart(weekStart);
  if (existing) return { report: existing, created: false };

  try {
    return await generateAndSaveReport({ weekStart, weekEnd });
  } catch (err) {
    console.error("[reports] auto-generate failed:", err.message);
    return null;
  }
}
