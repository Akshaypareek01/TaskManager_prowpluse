...
import { parseRoastKeywordsFromDb, serializeRoastKeywords } from "./roastKeywords.js";
...
/**
 * Update the signed-in user's hourly roast opt-in and/or keywords.
 * @param {string} userId
 * @param {{ allow?: boolean, keywords?: unknown }} prefs
 * @returns {Promise<object>}
 */
export async function updateRoastPreference(userId, prefs) {
  const { allow, keywords } = prefs ?? {};
  const updates = {};
  ...
  if (allow !== undefined) {
    ...
    updates.allowHourlyRoast = allow;
  }
  ...
  if (keywords !== undefined) {
    ...
    updates.roastKeywords = serializeRoastKeywords(keywords);
  }

  if (Object.keys(updates).length === 0) {
    throw new Error("allow or keywords is required");
  }
